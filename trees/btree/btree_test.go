...
func TestBTreeGet1(t *testing.T) {
	...
	for _, test := range tests {
		if value, found := tree.Get(test.key); value != test.value || found != test.flag {
			t.Errorf("Got %v,%v expected %v,%v", value, found, test.value, test.flag)
		}
	}
}
...
func TestBTreeLeftAndRight(t *testing.T) {
	...
	{
		actualValue, ok := tree.LeftKey()
		assert.True(t, ok)
		assert.Equal(t, 1, actualValue)
	}

	{
		actualValue, ok := tree.LeftValue()
		assert.True(t, ok)
		assert.Equal(t, "x", actualValue)
	}

	{
		actualValue, ok := tree.RightKey()
		assert.True(t, ok)
		assert.Equal(t, 7, actualValue)
	}

	{
		actualValue, ok := tree.RightValue()
		assert.True(t, ok)
		assert.Equal(t, "g", actualValue)
	}
}
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	...
	assert.Equal(t, []string{"x", "b", "c", "d", "e", "f", "g"}, tree.Values())
...
func TestBTreeSearch(t *testing.T) {
	{
		...
		tests := []option[int, int]{
			{0, 0, false},
		}
		for _, test := range tests {
			index, found := tree.search(tree.Root, test.key)
			if actualValue, expectedValue := index, test.value; actualValue != expectedValue {
				t.Errorf("Got %v expected %v", actualValue, expectedValue)
			}
			if actualValue, expectedValue := found, test.flag; actualValue != expectedValue {
				t.Errorf("Got %v expected %v", actualValue, expectedValue)
			}
		}
	}
...
func assertValidTree[K, V any](t *testing.T, tree *Tree[K, V], expectedSize int) {
...
func assertValidTreeNode[K, V any](t *testing.T, node *Node[K, V], expectedEntries int, expectedChildren int, keys []K, hasParent bool) {
	...
	for i, key := range keys {
		if actualValue, expectedValue := node.Entries[i].Key, key; !reflect.DeepEqual(actualValue, expectedValue) {
